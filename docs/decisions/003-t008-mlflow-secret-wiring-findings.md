...
**Status:** authored by the T008 `infra-scaffolder` dispatch, 2026-08-16, alongside `infra/terraform/20-platform/mlflow.tf` and `infra/helm-values/mlflow.yaml`. Not reviewed by `spec-guardian`/`peer-reviewer` yet. Per CLAUDE.md's "unknowns discovered mid-task: write a short note in `docs/decisions/` and surface to the operator; do not improvise architecture" — this is that note, not a unilateral architecture change.
