...
4. **OPEN — linter gap, own PR required.** `orbital_drift.traceability`'s `_NODE_ID` regex (src/orbital_drift/traceability.py:50) truncates a parametrized pytest id whose parameter tuple contains a slash (measured 2026-08-22: the TERRAFORM_IMAGE parametrization of the digest-lockstep test truncates at `hashicorp`, making that id un-citable as Green evidence — CI run 32574653365 red on exactly this). The regex already carries a comment about a prior `[`-truncation fix; `/` inside brackets is the next instance of the same defect class. Per the definition-of-done rule that gate logic is never edited in the same PR as its failing run, the fix goes in its own process PR with a decision-log line. Until then, Green rows must cite bracket-free node ids.
