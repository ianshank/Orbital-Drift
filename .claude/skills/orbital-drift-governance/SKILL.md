...
- **RB-011** (09-03): full-suite QA triage authorized on branch `qa/full-suite-triage` (main @ 84d45ed). Baselined every CI gate plus the GPU-gated live tiers (sanity/integration/e2e — executed for real, all passing) and an ad-hoc uncommitted live-STAC probe. Findings in `docs/incidents/2026-09-03-full-suite-triage.md`: a mypy `torch.amp` attr-defined pair (confirmed environmental by RB-011a — pinned `torch==2.13.0` already exports the names; no src/ change) and a confirmed Windows-only path-separator bug in `test_coverage_positive_control.py`. Fixes nothing and unlocks no src/ change; each approved finding ships as its own task/PR with TDD.
- **RB-011a** (09-03): execution record under RB-011 — both findings resolved same-day. Finding 1 (mypy `torch.amp`) needed no code change: the pinned `torch==2.13.0` source confirmed to already declare the correct `__all__`; the local host's stale nightly build lacked it. Finding 2 (Windows path-separator bug) got a v1 fix on `fix/coverage-positive-control-windows-path-sep` (668dc25), TDD-verified and unit-stage green at that step (946 passed, 0 failed); see RB-011b for the POSIX correction and final state (149cc85, 947 passed, 0 failed).
- **RB-011b** (09-03): execution record under RB-011 — PR #19's Linux CI caught a POSIX defect in the v1 fix: `Path(...).as_posix()` is a no-op for backslashes on POSIX, so the new backslash-keyed regression test passed on Windows but failed in CI. Corrected in 149cc85 with a string-level `key.replace("\\", "/")` normalization (deterministic on every OS) and the test parametrized over both separators; unit stage green (947 passed, 0 failed). PR #20's all-green CI independently confirms Finding 1's environmental resolution.

- **RB-012** (09-05): plan-artifact reconciliation after a five-lens deep peer review (`docs/decisions/013-plan-artifact-reconciliation.md`). `docs/development/NEXT_STEPS.md` declared Phase 1 "Completed" against tasks.md and its rollback drill named two nonexistent symbols; rewritten as the forward roadmap that ARCHITECTURE.md and RB-010 already cite by name but which was never committed. REFERENCE_GUIDE.md corrected (wrong coverage floor, a `--cov` command weaker than the gate, seven undeclared libraries presented as the as-built stack). Traceability: three of six success criteria carried another requirement's text and spec.md's SC-002 — the only performance budget in the spec — was traced nowhere; FR-011a had no row; four "confirmed by search" config claims falsified by eight importers. One new `traceability` rule (spec/matrix requirement-ID parity, honestly scoped: IDs only, NOT summary fidelity) plus the projections regex that could not see T001a/T001b/T004a. Adds tasks T053-T065 for RB-010 findings owned by no part, plus T063 for a live defect found by running the suite: `eval/calibration.py` can return an ECE of 1.5 (numpy broadcasts a length-1 weight array over 2 deviations instead of raising), which also makes its Hypothesis test redden CI intermittently on unrelated PRs. EXPLICIT LIMIT: creates those task IDs but authorizes the execution of NONE of them; flips no checkbox; changes no gate bar VALUE; creates no G-x; leaves the Principle II decision open.

- **RB-012a** (09-05): execution record under RB-012 — both reviewers returned BLOCK on its diff. The unenumerated items (Epic E6, stories S6.1-S6.13, the new `governance` label, the regenerated projections) STAND as mechanically forced by RB-012's own task block, since `test_every_agent_task_has_a_story` and the byte-comparing `projections` stage leave no choice. Round 1 added three WHEN/THEN scenarios to the governance-harness "Traceability matrix is linted" requirement (both reviewers ruled a new merge-blocking rule needs a requirement, per T001a/T001b), a PARTIALLY IMPLEMENTED annotation on the story-status scenario implemented by nothing, and tasks T064/T065. THE FINDING THAT MATTERED: RB-012's own parity rule FAILED OPEN on the defect it was written for — one indented bullet in spec.md disarmed it — now fail-closed and mutation-tested. Two findings rejected on measurement (the ECE failure is ~2-in-20, not deterministic; the budget question is escalated to the operator, not resolved). EXPLICIT LIMIT: authorizes the execution of no task, flips no checkbox, changes no gate bar.
