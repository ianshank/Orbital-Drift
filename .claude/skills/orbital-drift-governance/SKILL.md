...
- **RB-011a** (09-03): execution record under RB-011 — both findings resolved same-day. Finding 1 (mypy `torch.amp`) needed no code change: the pinned `torch==2.13.0` source confirmed to already declare the correct `__all__`; the local host's stale nightly build lacked it. Finding 2 (Windows path-separator bug) fixed on `fix/coverage-positive-control-windows-path-sep` (668dc25), TDD-verified, unit stage green (946 passed, 0 failed); not yet merged.
