...
- **RB-012** (09-03): full-suite QA round-2 triage authorized on `qa/full-suite-triage-r2` (main @ 4b6ac35, the PR #20 merge closing RB-011). All gate stages run individually, plus the GPU-gated live tiers against live docker-compose services and an ad-hoc live-STAC probe; the request's "live LLM API" verified absent from this codebase (no SDK deps, no calls in guard scripts) and discharged as live external surfaces generally — recorded as a report note, not a defect. RB-010 deferred items stay out of scope. Findings in `docs/incidents/2026-09-03-full-suite-triage-r2.md`. Fixes nothing and unlocks no src/ change; each approved finding ships as its own task/PR with TDD.
- **RB-012a** (09-03): execution record under RB-012 — round-2 closed same-day with ZERO new defects: 16 of 17 stages green (unit 949 passed, coverage 1159 passed at 98.89% global / ≥90% per-file), live GPU tiers 8/8 executed and passed, live-STAC probe 4 scenes in 0.52 s. The one red stage (`typecheck`) is round-1's Finding 1 exactly, mechanism re-verified on this host's torch nightly (no `__all__` in `torch/amp/__init__.py`); environmental, no code change, CI's pinned torch==2.13.0 passes the same source. No fix branches needed; report at `docs/incidents/2026-09-03-full-suite-triage-r2.md`.
- **RB-012b** (09-03): execution record under RB-012 — both findings closed. Finding 1 cured by verified venv alignment to pinned `torch==2.13.0+cu130` (CUDA real, `__all__` present). That stabilized the final `all` run, which surfaced Finding 2 — a real defect: `eval/calibration.py._bin_weights` diverged from sklearn on collapsed quantile boundaries, letting ECE exceed 1.0 (measured 1.5). Fixed TDD on `fix/calibration-ece-bin-weights` (1e2d5b3) with the deterministic regression `test_ece_never_exceeds_one_when_a_quantile_bin_collapses`; full `unit` 950 passed/0 failed. The fix branch carries the only src/ change.
