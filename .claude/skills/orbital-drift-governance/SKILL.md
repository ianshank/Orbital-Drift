...
- **RB-008c** (08-22): execution record for the gate-integrity FOLLOW-UP PR, kept as its own entry rather than editing merged RB-008b — the concurrency guard was scope-blind (a job-level `concurrency:` override reinstated the RB-008 incident at an unchanged suite result; operator authorized the third cycle under R-5), RB-008a(d)'s glob half recorded UNDISCHARGED with a named owner, `ci/checks.sh` silently ran one stage and exited 0 on several (outside RB-008's literal enumeration; the operator authorized that item explicitly in-session, logged per rule 6), and a `cancel-in-progress` failure was diagnosed with a grouping narrative. Follow-up inside PR-stream (1), not a fourth batch member: process track, outside the M0 budget on the RB-007(b) baseline. No gate bar VALUE changes; unlocks no feature task.
