...
1. `.specify/memory/constitution.md` — especially: Principle I (no cluster-mutating commands anywhere in agent-authored scripts/CI), Principle II (no code, metrics, or eval-harness logic imported from ianshank/Agents, Edge-DIT, or langfuse-eval-harness; only standard-library drift/eval methods; governance/process artifacts — charters, decision-log formats, review protocols, CI/hook/guard patterns, planning templates — are exempt per the v1.1.0 amendment), Principle III (no magic numbers — thresholds, cadences, names must come from config).
