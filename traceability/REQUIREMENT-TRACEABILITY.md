...
| FR-009 | FastAPI serving by registry stage; configurable canary | `src/orbital_drift/serve/app.py` | `tests/contract/test_serving.py` | M4 (T041, T042) | In-progress | RB-010 (2026-09-01): `/predict` + canary routing real and tested; `/metrics` is hand-rolled JSON, not real Prometheus exposition format — see T042. **Corrected 2026-09-05 (RB-012):** this note previously said `serve/app.py` "never imports `orbital_drift.config`". It does, at `serve/app.py:21`, since RB-010 Part 5b. **RB-015 Part H (2026-09-11):** the shipped image is liveness-healthy (`HEALTHCHECK` / compose / rollback-skill probe `/livez`); `/readyz` and `/healthz` stay 503 until a production model is loaded. T053 checkbox stays PARTIAL — model-load waits T059/D-2. Dummy weights at startup forbidden (D-015/D-02). |
...
| FR-012 | All thresholds/cadences/AOI/names from configuration | `src/orbital_drift/config.py` | `tests/unit/test_config.py` | M1 (T015) | In-progress | RB-010 (2026-09-01): `config.py` exists and is a real, tested `pydantic_settings.BaseSettings` (T015 DONE per tasks.md). **Corrected 2026-09-05 (RB-012):** this note previously claimed "zero `src/orbital_drift` modules import `orbital_drift.config`". RB-010 Parts 5a/5b/5c wired eight after Part 1 reconciled this file, and no later part revisited it — measured at HEAD: `ingest/{stac_client,tile_store,cloud}.py`, `data/dataset.py`, `drift/metrics.py`, `train/baseline.py`, `registry/ops.py`, `serve/app.py`. **RB-015 (2026-09-11):** T061 F3 landed in PR #27 — `drift/trigger.py` resolves `hysteresis_window`/`cooldown_scenes` from config. Residual: D-012 F1/F2/F4/F5 (Part F GATED until operator answers F2 `stride` and F5 `limit=10`), plus `data/lakefs_ops.py` still unwired, plus no composition root (`get_config()` is called from nowhere in `src/`). Constitution III; reviewer-enforced meanwhile |
...
| SC-004 | Rollback drill executed < 10 min, documented | (operations) | (drill record) | M3/M5 (T040) | Planned-gated | Operator-only. Blocked beyond T040: the rollback runbook (T039) is unwritten, and nothing loads a production model outside tests (T053/T059) -- so there is no canary to roll back. **RB-015 (2026-09-11):** T062 lock on `rollback_production` landed in PR #25 (`self._lock`); `get_stage_version` stays unlocked because wrapping it while the lock is held would deadlock. See `docs/development/NEXT_STEPS.md` section 5 |
