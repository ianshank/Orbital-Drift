...
- [x] T004 [A:runbook-writer] Runbook: k3s install (single node, GPU node labels, containerd nvidia runtime), `docs/runbooks/01-k3s-install.md`. **Checked off 2026-08-16** for the `runbook-writer` portion, after `spec-guardian` APPROVE and `peer-reviewer` APPROVE across two review rounds (see T002's entry for the shared findings — this runbook was the one with the platform-destroying rollback instruction and the unanchored containerd-handler grep, both fixed and re-verified). **`infra/k3s/config-v3.toml.tmpl` itself is NOT yet checked off** — this runbook's Step 2 STOP condition and the note below both still apply, and the template needs its own `infra-scaffolder` dispatch via the handoff note this PR's description carries. **Also populates `infra/k3s/`** — the containerd `config-v3.toml.tmpl` that D-000/D-02b requires, since k3s regenerates `config.toml` and hand-edits do not survive. That directory was declared in `plan.md`'s Project Structure and populated by no task in T001–T052, so `tests/unit/test_repo_structure.py` would have asserted its `.gitkeep` forever; giving it an owner is the fix, not deleting it, because the template is a real Constitution IV artifact. The template itself is an infra manifest, so `infra-scaffolder` authors it via an explicit handoff note in the PR description (CLAUDE.md's collaboration protocol) while `runbook-writer` owns the runbook that installs it. The runbook must carry D-000/D-02b's restart hazard — k3s scans PATH once per start, so a toolkit DaemonSet landing after k3s is up leaves the `nvidia` handler missing until `sudo systemctl restart k3s` — and D-000/D-02's rule that the NRI plugin stays **off**, since enabling it deletes the `nvidia` RuntimeClass that D-000/D-03's UUID pinning depends on.
